...
from .util import get_shops_uuid_user_id, \
    get_period, get_shops_user_id, get_shops
from pprint import pprint
from collections import OrderedDict
from .inputs import  GroupsInput, DocStatusInput, \
            ReportSalaryInput, ReportsSalarySettingInput, \
            ReportGroupUuidAccessoryInput, ShopInput,\
            ReportMotivationUuidInput, GroupInput, ProductInput, \
            ReportАssignSalaryInput, ReportMotivationInput, \
            EmployeesInput, ReportSurchargeInput
    ...
    if session.params["inputs"]['0']:
        ...
        if session.params["inputs"]['0']['reports'] == 'setting':
            if 'reports_salary-setting' in session.params["inputs"]['0']:
                ...
                if session.params["inputs"]['0']['reports_salary-setting'] == 'group_uuid_accessory':
                    if 'report' in session.params["inputs"]['0']:
                        # Назначить группы аксессуаров
                        if session.params["inputs"]['0']['report'] == 'assigning_group_uuid_accessory':
                            return {
                                'parentUuid': GroupsInput,
                                'docStatus': DocStatusInput,
                            }
                        # Запрос назначенных групп аксессуаров
                        if session.params["inputs"]['0']['report'] == 'get_group_uuid_accessory':
                            return {
                                "shop": ShopInput,
                        ...
                        return{
                            'report': ReportGroupUuidAccessoryInput
                        }
                # Добавление и просмотр мотивационого товара
                if session.params["inputs"]['0']['reports_salary-setting'] == 'motivation_uuid_accessory':
                    if 'report' in session.params["inputs"]['0']:
                        # Назначить товар доб. мотивации
                        if session.params["inputs"]['0']['report'] == 'product_ext_motivation':
                            if 'parentUuid' in session.params["inputs"]['0']:
                                ...
                                return {
                                    'parentUuid': GroupInput
                                }
                        # Запрос назначенных товаров доб. мотивации
                        if session.params["inputs"]['0']['report'] == 'get_product_ext_motivation':
                            return {
                                 "shop": ShopInput,
                            }
                    else:
                        return {
                            'report': ReportMotivationUuidInput
                        }
                # Назначение и просмотр окладов на ТТ
                if session.params["inputs"]['0']['reports_salary-setting'] == 'assigning_salary':
                    if 'report' in session.params["inputs"]['0']:
                        # Назначить оклады на ТТ
                        if session.params["inputs"]['0']['report'] == 'assigning_salary_':
                            return {
                                "shop": ShopInput,
                                'salary': SalaryInput,
                                'docStatus': DocStatusInput,
                        ...
                        return {
                            'report': ReportАssignSalaryInput
                        }
                # Назначение и просмотр суммы мотивации за вып. плана 
                if session.params["inputs"]['0']['reports_salary-setting'] == 'motivation':
                    if 'report' in session.params["inputs"]['0']:
                        # Назначить сум. за выпол. пл.
                        if session.params["inputs"]['0']['report'] == 'amount_of_motivation':
                            return {
                                'motivation': MotivationInput
                            }
                        # Запрос назначенной сум. за выпол. пл.
                        if session.params["inputs"]['0']['report'] == 'get_amount_of_motivation':
                            return {
                                "shop": ShopInput
                            }
                    else:
                        return {
                            'report': ReportMotivationInput
                        }
                # Назначение и просмотр сум. доплату к зп.
                if session.params["inputs"]['0']['reports_salary-setting'] == 'surcharge':
                    if 'report' in session.params["inputs"]['0']:
                        # Назначить доплату к зп
                        if session.params["inputs"]['0']['report'] == 'assign_a_surcharge':
                            return {
                                'uuid': EmployeesInput,
...
def generate(session: Session):
    ...
    if params['report'] == 'assigning_group_uuid_accessory':
        ...
        for shop_id in shops_id:
            ...
            GroupUuidAks.objects(shop_id=shop_id, closeDate=close_date, x_type='MOTIVATION_PARENT_UUID').update(**dict_, upsert=True)
    ...
    if params['report'] == 'get_group_uuid_accessory':
        ...
        documents = GroupUuidAks.objects(shop_id=shop_id_[0], x_type='MOTIVATION_PARENT_UUID').order_by("-closeDate").first()
    ...
    if params['report'] ==  'product_ext_motivation':
        ...
        for i in range(int(room) + 1):
            ...
            if 'uuid' in session.params["inputs"][str(i)]:
                ...
                motivationUuids.update(
                    {
                        session.params["inputs"][str(i)]['uuid']: int(session.params["inputs"][str(i)]['motivation'])
                    }
                )
        ...
        for shop_id in shops_id:
            ...
            GroupUuidAks.objects(shop_id=shop_id, closeDate=close_date, x_type='MOTIVATION_UUID').update(**dict_, upsert=True)
    ...
    if params['report'] ==  'get_product_ext_motivation':
        ...
        documents = GroupUuidAks.objects(shop_id=shop_id_[0], x_type='MOTIVATION_UUID').order_by("-closeDate").first()
    ...
    if params['report'] ==  'assigning_salary_':
        ...
        for i in range(int(room) + 1):
            ...
            GroupUuidAks.objects(shop_id=_dict['shop_id'], closeDate=close_date, x_type='SALARY').update(**_dict, upsert=True)
    ...
    if params['report'] ==  'get_salary':
        ...
        shop_id_ = shops['shop_id']
        shop_name = shops['shop_name']
        documents = GroupUuidAks.objects(shop_id=shop_id_[0], x_type='SALARY').order_by("-closeDate").first()
    ...
    if params['report'] ==  'amount_of_motivation':
        ...
        for shop_id in shops_id:
            ...
            GroupUuidAks.objects(shop_id=_dict['shop_id'], closeDate=close_date, x_type='MOTIVATION').update(**_dict, upsert=True)
    ...
    if params['report'] ==  'get_amount_of_motivation':
        ...
        shop_id_ = shops['shop_id']
        shop_name = shops['shop_name']
        documents = GroupUuidAks.objects(shop_id=shop_id_[0], x_type='MOTIVATION').order_by("-closeDate").first()
        if documents:
            return [
            {'ДАТА:': documents.closeDate},
    ...
    if params['report'] ==  'assign_a_surcharge':
        ...
        for i in range(int(room) + 1):
            ...
            employee_result.update(
                {session.params["inputs"][str(i)]['uuid']: session.params["inputs"][str(i)]['surcharge']} 
                )
            GroupUuidAks.objects(employee_uuid=session.params["inputs"][str(i)]['uuid'], closeDate=close_date, x_type='ASSING_A_SURCHARGE').update(**_dict, upsert=True)
    ...
    if params['report'] == 'get_surcharge':
        ...
        documents = GroupUuidAks.objects(employee_uuid=employee_uuid, x_type='ASSING_A_SURCHARGE').order_by("-closeDate").first()
