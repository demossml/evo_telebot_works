...
def generate(session: Session):
                        ...
                        if trans["x_type"] == "REGISTER_POSITION":
                            if trans["commodityUuid"] == uuid:
                                if documents.x_type == "SELL":
                                    if shop_name in result:
                                        result[shop_name] += (
                                            trans["balanceQuantity"] * cost_price
                                        )
                                        result[shop_name] -= (
                                            trans["quantity"] * cost_price
                                        )
                                    else:
                                        result[shop_name] = (
                                            trans["balanceQuantity"] * cost_price
    ...
    if params["report"] == "get_report":
        ...
        for shop_id in shops_uuid:
            ...
            result.append({"Магазин:": shop_name, "Остаток:": remainder_shop})
            # pprint(shop_name)
            ...
            if len(_dict) > 0:
                ...
                for prod in products:
                    for k, v in prod.items():
                        product_uuid = k
                        prod_name = (
                            Products.objects(uuid__exact=k, group__exact=False)
                            .only("name")
                            .first()
                            .name
                        )
