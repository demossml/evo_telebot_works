from bd.model import Shop, Products, Documents, Employees, \
    Session, ZReopt, Plan, Surplus
from arrow import utcnow, get
from .util import get_products, get_shops, get_period_day, get_period, generate_plan
from .inputs import ShopInput, PeriodDateInput, OpenDatePastInput, \
    CloseDatePastInput, ReportsZInput, OpenDatePast2Input, GroupInput, \
    ProductsSaleInput, ReportsZReport2Input, AfsInput, ReportsSurplusInput
    ...
    if session.params["inputs"]['0']:
        if session.params["inputs"]['0']['report'] == 'surplus':
            if 'report_surplus' in session.params["inputs"]['0']:
                ...
                return {
                    'report_surplus': ReportsSurplusInput
                }
...
def generate(session: Session):
    ...
    if 'report_surplus' in session.params["inputs"]['0']:
        if session.params["inputs"]['0']['report_surplus'] == 'register_surplus':
            ...
            Surplus.objects(closeDate=dict_['closeDate'], shop_id=dict_['shop_id']).update(**dict_, upsert=True)

            result_message = {
                '1 TT:': [i['name'] for i in Shop.objects(uuid=params['shop_id'])][0],
                '2 Излишек': '{} {}'.format(params['surplus'], '₽'),
                '3 ПРОДАВЕЦ': [i['name'] for i in Employees.objects(lastName=str(session.user_id))][0],
        ...
        if session.params["inputs"]['0']['report_surplus'] == 'get_surplus':
            ...
            documents = Surplus.objects(__raw__={
            ...
            for item in documents:
                dict_ = {
                    ...
                    '3 ПРОДАВЕЦ': [i['name'] for i in Employees.objects(lastName=str(item['user_id']))][0],
    ...
    if 'report_z' in session.params["inputs"]['0']:
        if session.params["inputs"]['0']['report_z'] == 'z_report':
            ...
            employees = [i.name for i in Employees.objects(lastName=str(session.user_id))]
            ...
            _dict = {
                ...
                'Расхождения терминала с кассой:': "{} ₽".format(session.params["inputs"]['0']['terminal']),
                ...
                'Зп мотивация:': "{} ₽".format(session.params["inputs"]['0']['salaryPromo']),
        ...
        if session.params["inputs"]['0']['report_z'] == 'get_z_report':
            ...
            dict_1 = {'photo_territory_1': documents['photoList1']['photo'],
                      'photo_territory_2': documents['photoList2']['photo']}

            shop_name = [i.name for i in Shop.objects(uuid=documents.shop)]
            employees = [i.name for i in Employees.objects(lastName=str(documents.user_id))]

            if len(employees) > 0:
                employeeName = employees[0]
            else:
                employeeName = ['Unknown ']
            salary_total = int(documents.salary) + int(documents.planSalary) + int(documents.aksSalary) + int(
                documents.salaryPromo)

            dict_ = {
                'Дата/Время:': documents.locationData[0:16],
                'Расхождения терминала с кассой:': "{} ₽".format(documents.terminal),
                'Оклад:': "{} ₽".format(documents.salary),
                # 'План(Fyzzi/Электро):': documents.plan,
                'Зп план:': "{} ₽".format(documents.planSalary),
                'Зп акс:': "{} ₽".format(int(documents.aksSalary)),
                'Зп мотивация:': "{} ₽".format(documents.salaryPromo),
                'Итого зп:': "{} ₽".format(round(salary_total, 0)),
                'ТТ:': shop_name[0],
                'Продавец:': employeeName
            }
            result.append(dict_)
            if shop in shops_id_2:
                shop_id = [shop, shops_id_2[shop]]
            else:
                shop_id = [shop]
            register = 1
                        ...
                        for trans in doc['transactions']:
                            if trans['x_type'] == 'FPRINT_Z_REPORT':
                                _dict['✅{} Магазин:'.format('1').upper()] = \
                                    [i.name for i in Shop.objects(uuid=doc['shop_id'])][0]
                                _dict['✅{} ДАТА/ВРЕМЯ ЗАКРЫТИЯ:'.format('2')] = get(doc['closeDate']).shift(
                                    hours=3).isoformat()[0:16]

                                employees = Employees.objects(uuid=doc['openUserUuid']).first()
                                last_name = employees.lastName
                                name_ = employees.name
                                _dict['✅{} ПРОДАВЕЦ:'.format('3')] = '{} {}:'.format(last_name, name_).upper()

                                _dict['✅{} ПРОДАЖИ {}:'.format('4',
                                                               trans['sales']['sections'][1]['name'])] = '{}₽'.format(
                                    trans['sales']['sections'][1]['value'])
                                _dict['✅{} ПРОДАЖИ {}:'.format('5',
                                                               trans['sales']['sections'][2]['name'])] = '{}₽'.format(
                                    trans['sales']['sections'][2]['value'])
                                _dict['✅{} ВОЗВРАТЫ {}:'.format('6',
                                                                trans['salesBack']['sections'][1][
                                                                    'name'])] = '{}₽'.format(
                                    trans['salesBack']['sections'][1]['value'])
                                _dict['✅{} ВОЗВРАТЫ {}:'.format('7',
                                                                trans['salesBack']['sections'][2][
                                                                    'name'])] = '{}₽'.format(
                                    trans['salesBack']['sections'][2]['value'])
                                _dict['✅{} ИТОГО ПРОДАЖИ:'.format('8')] = '{}₽'.format(trans['sales']['summ'])
                                _dict['✅{} НАЛИЧНЫМИ В КАССЕ:'.format('9')] = '{}₽'.format(trans['cash'])
                                _dict['✅{} ИТОГО ВЫПЛАТЫ:'.format('10')] = '{}₽'.format(trans['cashOut'])

                    if doc['x_type'] == 'CASH_OUTCOME':
                        for trans in doc['transactions']:

                            if trans['x_type'] == 'CASH_OUTCOME':
                                _dict['✅ВЫПЛАТА ЧЕК №{}'.format(doc['number'])] = '{}₽/{}'.format(trans['sum'],
                                                                                                  payment_category[
                                                                                                      str(trans[
                                                                                                              'paymentCategoryId'])])
                    ...
                    if doc['x_type'] == 'CASH_INCOME':
                        for trans in doc['transactions']:
                            if trans['x_type'] == 'CASH_INCOME':
                                _dict['✅ВНЕСЕНИЕ ЧЕК №{}'.format(doc['number'])] = '{}₽'.format(trans['sum'])
        ...
        if session.params["inputs"]['0']['report'] == 'report_cash_outcome':
            ...
            for shop_uuid in shop_id:
                ...
                for doc in documents:
                    ...
                    if doc['x_type'] == 'CASH_OUTCOME':
                        ...
                        for trans in doc['transactions']:
                            if trans['x_type'] == 'CASH_OUTCOME':
                                employees = Employees.objects(uuid=trans['userUuid']).first()
                                last_name = employees.lastName
                                name_ = employees.name
                                result.append(
                                    {
                                        ...
                                        'Пордавец:'.upper(): '{} {}:'.format(last_name, name_).upper(),
                                        'Категории платежа:'.upper(): payment_category[str(trans['id'])],
                                        'Сумма:'.upper(): '{} ₽'.format(trans['sum'])
                                    }
                                )
                                sum_payment_category['sum'] += trans['sum']
                                if str(trans['paymentCategoryId']) in sum_payment_category:
                                    sum_payment_category[str(trans['paymentCategoryId'])] += trans['sum']
                                else:
                                    sum_payment_category[str(trans['paymentCategoryId'])] = trans['sum']
            ...
            for k, v in sum_payment_category.items():
                if k != 'sum':
                    _dict.update(
                        {
                            payment_category[k]: '{} ₽'.format(v)
                        }
                    )
            _dict.update(
                {
                    'Сумма:'.upper(): '{} ₽'.format(sum_payment_category['sum'])
                }
            )
            result.append(_dict)
        ...
        if session.params["inputs"]['0']['report'] == 'get_check':
            ...
            for doc in documents:
                dict_ = {'ЧЕК №:': doc['number'],
                         'ДАТА:': since[0:10]
                         }
                for trans in doc["transactions"]:
                    if trans["x_type"] == "REGISTER_POSITION":
                        dict_.update({
                            '{}'.format(trans['commodityName']): '{} {}'.format(trans['quantity'], 'шт.')
                        })
