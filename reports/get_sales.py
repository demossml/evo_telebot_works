...
from .inputs import ReportSalesInput, ShopAllInput, GroupInput, \
    PeriodDateInput,OpenDatePastInput, CloseDatePastInput

name = "🛒 ОТЧЕТЫ ПО ПРОДАЖАМ ➡️".upper()
...
def get_inputs(session: Session):
    ...
    if session.params["inputs"]['0']:
        ...
        if session.params["inputs"]['0']['report'] == 'get_sales_by_shop_product_group_rub':
            shop_uuid = get_shops_user_id(session)
            if len(shop_uuid) > 0:
                return {
                    "shop": ShopAllInput,
        ...
        if session.params["inputs"]['0']['report'] == 'get_sales_by_shop_product_group_unit':
            shop_uuid = get_shops_user_id(session)
            if len(shop_uuid) > 0:
                return {
                    "shop": ShopAllInput,
        ...
        for since_, until_ in intervals:
            ...
            for shop in shop_id:
                ...
                if documents_open_session:
                    employees = Employees.objects(uuid=documents_open_session.closeUserUuid).first()
                    ...
                    for doc in documents:
                        for trans in doc["transactions"]:
                            ...
                            if trans["x_type"] == "PAYMENT":
                                payment_type_sum_sell[trans['paymentType']] += trans['sum']
                                payment_type_sum_sell_total[trans['paymentType']] += trans['sum']
